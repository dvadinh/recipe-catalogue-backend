...
@Service("stepAuthorizationService")
public class StepAuthorizationService {

  public boolean preAuthorizeUpdateMediaByStep(
    ...
    if (principal.getId().equals(step.getSection().getRecipe().getOwner().getId())) {
      return true;
    }

    throw new AuthorizationException(StepErrorDictionary.STEP_ACCESS_DENIED_001);

  }

  public boolean preAuthorizeDeleteMediaByStep(
      UserPrincipal principal,
      Section section
  ) {
